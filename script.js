...
const STORAGE_KEY = 'personalFinanceTracker_v2'; // Kunci LocalStorage Utama (Piutang)

// KODE NOTIFIKASI DIHAPUS


// ===================================
// 1. MANAJEMEN DATA & LOCAL STORAGE (PIUTANG/UTANG)
// ===================================
...
// ===================================
// KODE BARU: FUNGSI BUDGETKU (HARIAN)
// ===================================
const STORAGE_KEY_BUDGETKU = 'budgetKuTracker_v1';
let budgetKuTransactions = [];

const budgetKuBalanceEl = document.getElementById('current-balance-budgetku');
const budgetKuForm = document.getElementById('transaksi-form-budgetku');
const budgetKuList = document.getElementById('riwayat-budgetku');
const budgetKuClearBtn = document.getElementById('clear-budgetku-history');

// Format Rupiah (bisa pakai ulang, tapi kita buat mini-helper)
function formatCurrencyBudgetKu(amount) {
    return 'Rp ' + (amount).toLocaleString('id-ID');
}

// Render BudgetKu
function renderBudgetKu() {
    if (!budgetKuBalanceEl) return; // Jangan render jika elemen tidak ada

    budgetKuList.innerHTML = '';
    let budgetKuBalance = 0; // Hitung ulang saldo dari awal setiap render
    
    // Hitung saldo berdasarkan semua transaksi
    budgetKuTransactions.forEach(tx => {
        if (tx.type === 'pemasukan') {
            budgetKuBalance += tx.amount;
        } else {
            budgetKuBalance -= tx.amount;
        }
    });
    
    // Urutkan dari terbaru untuk ditampilkan
    const transactionsToRender = [...budgetKuTransactions].reverse();
    ...
    transactionsToRender.forEach(tx => {
        ...
        li.innerHTML = `
            <div>
                ...
                <div style="font-size: 0.8em; color: var(--text-muted);">${new Date(tx.id).toLocaleDateString('id-ID')}</div>
            </div>
            <span>${sign} ${formatCurrencyBudgetKu(tx.amount)}</span>
            <button class="delete-btn-budgetku" onclick="deleteBudgetKuTransaction(${tx.id})"><i class="fas fa-trash-alt"></i></button>
        `;
        budgetKuList.appendChild(li);
    });

    budgetKuBalanceEl.textContent = formatCurrencyBudgetKu(budgetKuBalance);
    
    // Set warna saldo
    if (budgetKuBalance < 0) {
        budgetKuBalanceEl.style.color = '#ffc0cb'; // Light red di box biru
    } else if (budgetKuBalance > 0) {
        budgetKuBalanceEl.style.color = '#c8e6c9'; // Light green di box biru
    } else {
        budgetKuBalanceEl.style.color = 'white'; // Warna default
    }
}

// Simpan BudgetKu
function saveBudgetKu() {
    localStorage.setItem(STORAGE_KEY_BUDGETKU, JSON.stringify(budgetKuTransactions));
}

// Muat BudgetKu
function loadBudgetKu() {
    const stored = localStorage.getItem(STORAGE_KEY_BUDGETKU);
    if (stored) {
        try {
            budgetKuTransactions = JSON.parse(stored);
            if (!Array.isArray(budgetKuTransactions)) budgetKuTransactions = [];
        } catch(e) {
            budgetKuTransactions = [];
        }
    }
    renderBudgetKu();
}

// Tambah Transaksi BudgetKu
function addBudgetKuTransaction(e) {
    e.preventDefault();
    
    const amount = parseFloat(document.getElementById('jumlah-budgetku').value);
    const type = document.getElementById('tipe-budgetku').value;
    const desc = document.getElementById('deskripsi-budgetku').value;

    if (isNaN(amount) || amount <= 0 || desc.trim() === '') {
        alert('Data tidak valid. Pastikan Jumlah dan Deskripsi terisi.');
        return;
    }
    ...
    budgetKuTransactions.push(transaction);
...
// Hapus Transaksi BudgetKu
function deleteBudgetKuTransaction(id) {
    if (confirm('Yakin ingin menghapus transaksi harian ini?')) {
        budgetKuTransactions = budgetKuTransactions.filter(tx => tx.id !== id);
        saveBudgetKu();
        renderBudgetKu();
    }
}

// Hapus Semua Riwayat BudgetKu
function clearBudgetKuHistory() {
     if (confirm('PERINGATAN! YAKIN INGIN MENGHAPUS SEMUA riwayat BudgetKu? Aksi ini tidak dapat dibatalkan.')) {
        budgetKuTransactions = [];
        saveBudgetKu();
        renderBudgetKu();
    }
...
// Inisialisasi Event Listener BudgetKu
// Kita panggil ini di DOMContentLoaded
function initBudgetKuListeners() {
    if (budgetKuForm) {
        budgetKuForm.addEventListener('submit', addBudgetKuTransaction);
    }
     if (budgetKuClearBtn) {
        budgetKuClearBtn.addEventListener('click', clearBudgetKuHistory);
    }
}

// ===================================
// 2. FUNGSI UTILITAS DATA (PIUTANG/UTANG)
// ===================================
...
function getFinancialTotals() {
    ...
    return {
        ...
        totalUtangAwal: Math.round(totalAwalUtang), 
...
function navigateTo(pageId) {
    ...
    if (pageId === 'homePage' || pageId === 'piutangPage' || pageId === 'utangPage') {
...
function reRenderActivePage(pageId) {
    if (pageId === 'homePage') {
        ...
        renderChart();
        // Pemanggilan update status notifikasi dihapus
        ...
        renderBackupPageData();
    } else if (pageId === 'budgetKuPage') {
        loadBudgetKu(); // Muat dan render data BudgetKu saat halaman dibuka
    }
}
...
// ===================================
// 3. FUNGSI BACKUP DAN RESTORE BARU (PIUTANG/UTANG)
// ===================================
...
function renderBackupPageData() {
    ...
    if (transactions && transactions.length > 0) {
        ...
        dataStatusEl.textContent = 'Data transaksi (Piutang/Utang) tidak ditemukan di Local Storage atau data kosong.';
...
function exportData() {
    ...
    if (!dataStr || dataStr === '[]') {
        alert('Tidak ada data Piutang/Utang yang dapat di-export!');
        return;
    }
    ...
    alert(`Data Piutang/Utang berhasil di-export sebagai: ${filename}`);
}
        ...
        try {
            ...
            if (Array.isArray(importedData)) {
                ...
                const confirmRestore = confirm(
                    `Anda yakin ingin memulihkan data Piutang/Utang? Data Anda saat ini (${transactions.length} transaksi) akan ditimpa dengan data dari file backup (${importedData.length} transaksi).`
                );
                ...
                if (confirmRestore) {
                    ...
                    alert('Data Piutang/Utang berhasil di-import dan dipulihkan! Halaman akan dimuat ulang untuk merefresh semua data.');
...
// ===================================
// 4. FUNGSI FORM & MODAL (PIUTANG/UTANG)
// ===================================
...
// ===================================
// 5. FUNGSI NOTIFIKASI OTOMATIS (DIHAPUS)
// ===================================

// Seluruh kode notifikasi (requestNotificationPermission, sendNotification, checkAndSendNotifications, dll.)
...
function exportToCSV() {
    if (transactions.length === 0) {
        alert('Tidak ada data Piutang/Utang untuk diexport.');
        return;
    }
...
function handleBackKey(event) {
    if (event.key === 'Escape' || event.type === 'backbutton') {
        ...
        else if (document.getElementById('sideMenuModal').style.display === 'block') {
            closeSideMenu();
            return; 
        }
        ...
        if (activePageId && activePageId !== 'homePage') {
             ...
             if (confirm("Apakah Anda yakin ingin keluar dari aplikasi?")) {
                 if (typeof navigator.app !== 'undefined' && typeof navigator.app.exitApp === 'function') {
                     ...
                     // Tidak bisa keluar (di web), biarkan kosong
                 }
             }
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    loadTransactions(); // Muat data Piutang/Utang
    calculateDueDate(); 
    ...
    // Setup Event Listener Backup/Restore (Piutang/Utang)
    ...
    // Inisialisasi BudgetKu (Harian)
    initBudgetKuListeners(); 
    ...
    // Initialize Notification Status and Scheduler (DIHAPUS)
});
